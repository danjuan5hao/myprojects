...
class BasicPreprocessor:
    ...
    def build_vocab(self,
                    ...
                    min_count: int = 3,
                    special_token: str = 'standard') -> Tuple[Dict[str, int], Dict[str, int], Dict[int, str]]:
    ...
    def build_embedding(self,  
        ...
        # # EMBEDDING（不管是否是为预训练的词/字向量） 是较为简单的一件事情， 只要通过
        # 【1. embed_type/file_path , 2. vocab, 5. speical_token】 就能解决预训练向量
        # 【3. corpus， 4. embedding_dim, 5. speical_token】 就可以随机初始化向量
        ...
        Args:
            ...
            special_token: str. 'standard' or 'bert': determine how to handle special tokens.
                ...
                special tokens.
        """
        zero_init_indices = vocab.get(self.pad_token)  # padding的部分词向量为零，就能保证之后不参与运算 # TODO 确认不参与运算
        if special_token == 'standard':
            rand_init_indices = vocab.get(self.unk_token)  # 这些mark的embedding 应该随机初始化 # TODO 为什么随机初始化
        elif special_token == 'bert':
        ...
        if embed_type == 'word2vec':
            ...
            try:
                ...
                raise ValueError('Argument `embed_type` input error: {}'.format(embed_type)) # 把embedding的东西屏蔽到外部。
