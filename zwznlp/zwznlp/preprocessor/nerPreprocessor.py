...
class NerPreprocessor(BasicPreprocessor):
    ...
    def __init__(self,
        ...
        assert self.use_char or self.use_bert, "must use char or bert embedding as main input" # 这里屏蔽了空输入
        special_token = 'bert' if self.use_bert else 'standard'

        self.bert_pretrain_weight = "chinese_wwm_pytorch"

        # build char vocabulary and char embedding
        if self.use_char:
            self.char_vocab_count, self.char_vocab, self.id2char = self.build_vocab(self.train_data, self.min_count, special_token)
